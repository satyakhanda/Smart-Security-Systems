...
def get_analytics_labels(video):
    ...
    for i, object in enumerate(video.metadata['objects']):
        labels.append(object)
        for timestamp in video.metadata['objects'][object]:
            x.append(timestamp)
            y.append(i)
    ...
    path = os.path.join(settings.MEDIA_ROOT, 'labels' + str(video.pk) + '.png')
    plt.savefig(path)
    return path
